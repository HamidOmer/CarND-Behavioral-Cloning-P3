...
lines_my = []
# Load My Data
with open('./My_Data_Orig/driving_log.csv') as csvfile:
	reader = csv.reader(csvfile)
	for line in reader:
...
model.fit_generator(train_generator, samples_per_epoch=(len(Xt_Images)//batchsz)*batchsz, validation_data=validation_generator,nb_val_samples=len(Xv_Images), nb_epoch=6)

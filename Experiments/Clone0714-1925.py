...
# Works fine on track 1. Does not work on track 2
...
def preprocess_image_train(image, steering):
	...
	while 1: # Loop forever so the generator never terminates
		...
		for offset in range(0, num_samples, batch_size):
			...
			for image,steering in zip(batch_images, batch_steerings):
				image, steering = preprocess_image_train(image, steering)
...
#	batch_steerings = []
	while True: # Loop forever so the generator never terminates
		...
		for i in range(batch_size):
			...
			keep_pr = 0
			while keep_pr == 0:
				#X, y = preprocess_image_train(pimg, pstr)
				X, y = preprocess_image_train(lcr_images[i_rand], str_angles[i_rand])
...
def val_generator(lcr_images, str_angles, batch_size=32):
	num_samples = len(lcr_images)
	while 1: # Loop forever so the generator never terminates
		shuffle(lcr_images, str_angles)
		for offset in range(0, num_samples, batch_size):
...
model.fit_generator(train_generator, samples_per_epoch=(len(Xt_Images)//batchsz)*batchsz, validation_data=validation_generator,nb_val_samples=len(Xv_Images), nb_epoch=3)
